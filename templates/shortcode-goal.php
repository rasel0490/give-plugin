...
<div class="give-goal-progress">
	<?php if ( ! empty( $show_text ) ) : ?>
		<div class="raised">
			...
			$formatted_goal1 = str_replace(',', '', $formatted_goal);
			$formatted_goal2 = str_replace('$', '', $formatted_goal1);
			preg_match_all('!\d+!', $formatted_goal2, $formatted_goal_final);
			$a = (int)$formatted_goal_final[0][1];
			
			
			$formatted_income1 = str_replace(',', '', $formatted_income);
			$formatted_income2 = str_replace('$', '', $formatted_income1);
			preg_match_all('!\d+!', $formatted_income2, $formatted_income_final);
			$b = (int)$formatted_income_final[0][1];
			...
			$togo_final = $a - $b;
				...
				echo sprintf(
					...
					__( $args['all_campain_amount'] ? '<div class="camp-money-stat" style="width: revert;margin-left: revert;margin-right: revert;">
                               <!-- goals -->
                               <div>
                                   <h6 class="camp-money-title" style="padding-left: 13px;">Goals</h6>
                                   <p class="camp-money-val">'.$formatted_goal.'</p>
                               </div>
                               <div class="vert-line"></div>
                               <!-- raised -->
                               <div>
                                   <h6 class="camp-money-title" style="padding-left: 13px;">Raised</h6>
                                   <p class="camp-money-val">'.$formatted_income.'</p>
                               </div>
                               <div class="vert-line"></div>
                                <!-- to go -->
                                <div>
                                    <h6 class="camp-money-title" style="padding-left: 13px;">To go</h6>
                                    <p class="camp-money-togo" style="text-align: center;">$'.$togo_final.'</p>
                                </div>
                           </div>' : '<span class="income" data-amounts="%1$s">%2$s</span> of <span class="goal-text" data-amounts="%3$s">%4$s</span> raised', 'give' ),
